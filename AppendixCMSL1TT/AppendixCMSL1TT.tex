...
\begin{landscape}
	\begin{figure}[htbp]{23cm}
		...
		\source{CERN (HiLumi HL-LHC project), 2015.}
...
\begin{figure}[htbp]{15cm}
	\caption{Muon road efficiency versus pile up.}
	\centering
	\subfloat[]{\includegraphics[width=7.5cm,height=7.5cm]{AppendixCMSL1TT/luciano_mupu_sf0p8_plot}}
	\subfloat[]{\includegraphics[width=8cm,height=8cm,trim={0cm 0.2cm 0cm 0cm},clip]{AppendixCMSL1TT/figs/muon_road_eff_vs_pile_up_deltaS}}
	\legend{(a) is a result without $\Delta S$ approach (done by another person in the CMS L1TT working group); (b) is the same type of event (muon + PU) with the $\Delta S$ (done by the author).}
	\source{The author, 2016.}
...
\begin{figure}[htbp]{16cm}
	...
	\legend{(a) Pulsar-IIB board; (b) inspection of Pulsar-IIB components; (c) and (d) the setup for measuring data transmission quality from the PRMs via optical cables; (e) crate containing Pulsar-IIB boards and PRMs (not visible since Pulsar boards sit right besides each other) with full setup ready for demonstration of the approach.}
