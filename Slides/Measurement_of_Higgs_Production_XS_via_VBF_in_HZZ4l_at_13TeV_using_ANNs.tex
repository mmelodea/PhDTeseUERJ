...
\begin{frame}{$Z+X$ Background Estimation}
	...
	\vspace{-0.3cm}
	\begin{multicols}{2}
	\begin{varblock}[5.8cm]{\center 2P2F CR}
		\begin{overpic}
			[scale=0.135,trim={1cm 0cm 2cm 1cm},clip]{figs/m4l_2p2f_4mu_smhiggs}
			\put(40,40){$\frac{f_{3}}{1-f_{3}}+\frac{f_{4}}{1-f_{4}}$}
...
\begin{frame}{$Z+X$ Background Estimation}
	...
	\begin{multicols}{2}
	\begin{minipage}{9cm}
		\begin{varblock}[8.2cm]{\centering $Z+X$ at SR}
			\centering
...
\begin{frame}{Artificial Neural Networks (ANNs)}
	\vspace{-0.1cm}	
	\begin{itemize}
		\item Training a ANN consists in finding parameters ($w_{i}$ and $b_{i}$) for $\Omega(x)$, such that it models a dataset in the best (possible) way;
...
\begin{frame}{Introduction}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item the author studied and implemented new components on the available software: synthetic match, duplicate removal, stub bending, road and combination truncation and, track fitter $\chi^{2}$ adjustment;
			\item results have been produced with different high-lumi scenarios (2-10k events): ($\mu$/$\pi$/$e$)+PU(140,200,300,400), $\nu$+PU(140,200,250), $t\bar{t}$+PU200 and jets($p_{T}=250GeV$)+PU200;
...
\begin{frame}{Simulation Studies: Stub Bending ($\Delta S$)}
	\begin{itemize}
		...
		\begin{itemize}
			\item Up to 14$\%$ and 50$\%$ of efficiency can be recovered when truncation is applied;
		\end{itemize}
	\end{itemize}
	\centering
	\begin{overpic}
...
\begin{frame}{Simulation Studies: Duplicate Removal}
	\begin{itemize}
		...
		\end{multicols}		
		\item The duplicate removal is a pure analytical procedure:
		\begin{itemize}
