	...
	\begin{multicols}{2}
		...
		\begin{minipage}{2.8cm}
			\begin{varblock}[2.8cm]{\center VBF - Signal}
				\begin{figure}
					\subfloat[$q\bar{q} \rightarrow Hq\bar{q}$]{\includegraphics[scale=0.05,trim={0cm 0cm 0cm 1cm},clip]{figs/vbf_diagram}}
				\end{figure}
			\end{varblock}		
		\end{minipage}		
	\end{multicols}
...
\begin{frame}{Projections for Future Luminosities}
	...
	\vspace{-0.5cm}
	\begin{itemize}
		\item Significance of 5.1$\sigma$ is expected for a luminosity 10x larger than the present one;
	\end{itemize}
	\vspace{-0.4cm}
	\begin{table}
...
\begin{frame}{Simulation Studies: Stub Bending ($\Delta S$) - The Edge of the Montain}
	...
	\begin{itemize}
		...
		\centering
		\includegraphics[scale=0.4]{CMSL1TTfigs/luciano_mupu_sf0p8_plot}
	\end{itemize}
	\vspace{0.2cm}		
	\begin{itemize}
		\item The author decided to check that and apply the $\Delta S$ approach (not considered by them at that time):\\
		\centering
		\includegraphics[scale=0.28]{CMSL1TTfigs/muon_road_eff_vs_pile_up_sf0p6}\\
		\includegraphics[scale=0.28]{CMSL1TTfigs/muon_road_eff_vs_pile_up_deltaS}	
	\end{itemize}		
	\end{multicols}	
\end{frame}
...
\begin{frame}{Simulation Studies: Stub Bending ($\Delta S$) - The Edge of the Montain}
	\begin{multicols}{2}
		\begin{itemize}
			...
			\centering
			\begin{overpic}
				[scale=0.4]{CMSL1TTfigs/luciano_mupu_sf0p8_plot}
				\put(20,0){
...
\begin{frame}{Simulation Studies: Synthetic Efficiency}
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Good}: first reco track with smallest $\chi_{match}$ and < $\bar{q}$;
			\item \textbf{Duplicate}: other reco tracks with $\chi_{match}$ < $\bar{q}$ but $\chi_{match}$ $\geq$ $\chi_{match}^{smallest}$;
			\item \textbf{Fake}: any reco track with $\chi_{match}$ $\geq$ $\bar{q}$;
		\end{itemize}
		\item In order to define the value $\bar{q}$ a tracking match based on stubs was carried out along with the synthetic efficiency:
		\begin{itemize}
			\item Scanning the cut on $\chi_{match}$ one checks (on a dedicated MC sample) the number of original stubs present in the reco track;
			\item Then, reduces the Fake rate and increases the Good rate as much as possible, avoiding random stub combinations (GOOD <5S) to pass the $\chi_{match}$ cut;
...
\begin{frame}{Simulation Studies: Duplicate Removal}
	\begin{itemize}
		...
		\end{multicols}		
		\item The duplicate removal is a stub-based mechanism:
		\begin{itemize}
			\item A reco track is taken from the reco tracks list (\textbf{A}) and inserted on a new tracks list (\textbf{A});
			\item Then, a loop is done over the remaining tracks on list \textbf{A}:
			\begin{itemize}
			...
			\end{itemize}
			\item The tracks remaining in the list \textbf{B} are the final tracks;
		\end{itemize}
		\item The DR mechanism was studied in order to tune the minimum number of stubs which allows massive remotion of duplicated tracks and keeps the track reco efficiency high;
	\end{itemize}
\end{frame}	
...
\begin{frame}{Simulation Studies: Final FOMs - $\mu$ in $t\bar{t}+PU200$}
	\vspace{-0.4cm}
	\begin{figure}
		\caption{Track reconstruction efficiency for a $\mu$ in $t\bar{t}+PU200$ sample. The pattern bank used had 64k patterns and truncation at 200 roads and 500 combinations has been applied. Duplicate removal was applied by requiring DR=0.}
		\centering
		\begin{overpic}
...
\begin{frame}{Simulation Studies: Final FOMs - $\mu+PU400$}
	\vspace{-0.4cm}
	\begin{figure}
		\caption{Track reconstruction efficiency for $\mu+PU400$ sample. The pattern bank used had 64k patterns and truncation at 200 roads and 500 combinations has been applied. Duplication removal was applied by requiring DR=0.}
		\centering
		\begin{overpic}
...
\begin{frame}{Theoretical Foundation}
	...
	\vspace{-0.6cm}
	\begin{itemize}
		\item $\mathcal{P}(x|\alpha)$: an event probability;
		\item $\sigma_{\alpha}$: process cross-section;
		\item $d\Phi$: analysis/detector acceptance;
		\item $f(\omega_{i})$: Parton Distribution Function;
		\item $\mathcal{M}_{\alpha}(y)$: matrix element;
		\item $W(x,y)$: transfer function;
	\end{itemize}
	\end{varblock}
	\end{multicols}
\end{frame}
...
\begin{frame}{Theoretical Foundation}
	...
	\begin{overpic}
		...
		\put(66,17){{\color{black}$\bullet$}\hspace{0.07cm} test}
		...
		\put(110,16){
			\begin{overpic}[scale=0.16]{FastMEfigs/psbD_vs_psbW}
				\put(5,70){\footnotesize $P_{SB}^{D} = \dfrac{D^{Bkg}_{Min}}{D^{Bkg}_{Min}+D^{Sig}_{Min}}$}
				\put(50,70){\footnotesize $P_{SB}^{W} = \dfrac{W^{Bkg}_{D_{min}}}{W^{Bkg}_{D_{min}}+W^{Sig}_{D_{min}}}$}
			\end{overpic}
		}
	\end{overpic}
	\end{figure}
...
\begin{frame}{Simulation Studies: Bias from Pattern Bank Size}
	\begin{itemize}
		\item The first point addressed during the development of the project was the influence of the pattern banks size:
	\end{itemize}
	\vspace{-0.3cm}
	\begin{figure}
	\caption{On graphs (a) and (b) the background pattern bank has a fixed size while the signal one is varied. On graphs (c) and (d) the opposite case is shown. Note, here "purity" is computed using the absolute number of events (without normalization).}
	\begin{overpic}
...
\begin{frame}{Simulation Studies: Impact of $\phi$ Variable}
	...
	\end{itemize}
	\begin{figure}
		\caption{Impact on the $P_{SB}^{D}$ distribution for signal and background due to use or not of $\phi$ in the computation of the distances between particles and events.}
		\begin{overpic}
...
\begin{frame}{Simulation Studies: Scaling $v_{k}$'s Contribution}
	\begin{itemize}
		...
		\begin{figure}
			\caption{Dependency on the variation of the fraction of signal events being mis-classified as background in function of the $\delta p_{T}$. A fixed value of 5.0 was used as the $\delta \eta$.}
			\begin{overpic}
...
\begin{frame}{Simulation Studies: \textit{Fast}ME Applied to HZZ4L CMS Data}
	\begin{itemize}
		...
		\item The results have been compared to the formal CMS discriminant, the so called MELA;
	\end{itemize}
\begin{figure}
	\caption{(a) Observed events classified as signal and background by the \textit{Fast}ME, (b) $P_{SB}^{D}$ and (c) MELA discriminants distribution versus the $m_{4l}$.}
	\flushleft
	\begin{overpic}
...
\begin{frame}{The \textit{Fast}ME Package}
	\begin{multicols}{2}
		\includegraphics[scale=0.24]{FastMEfigs/fastme_github_page}
		\begin{itemize}
			...
			\item During the author's first travel to Fermilab, this package was created and stored on GitHub:
		\end{itemize}
	\end{multicols}		
\end{frame}
...
\begin{frame}{Systematic Uncertainties Impact on $\mu_{qqH}$ Fit}
	\centering
	\includegraphics[scale=0.55]{figs/impacts_obs_4l_combination}
\end{frame}
