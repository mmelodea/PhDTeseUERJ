...
\begin{frame}{Artificial Neural Networks (ANNs)}
	\begin{itemize}
		\justifying
		\item ANNs unctionals coming from Machine Learning field (closest field to development of Artificial Intelligence) in Computer Science;
	...
	\end{itemize}
	\begin{itemize}
		\item An ANN is a composite of multidimensional parameterized functions in terms of weights ($w_{i}$) and biases ($b_{i}$), in the form: $\Omega(x) =  \Phi(f_{1}(x), f_{2}(x), ..., f_{n}(x))$;
	\end{itemize}
	\center		
...
\begin{frame}{Training Strategy}
	\begin{itemize}
		\justifying
		\item Each trained ANN model was produced with some checking plots, which were used to validate them;
		\item Here are an example of such plots:
	\end{itemize}
	\centering
	\begin{tabular}{ccc}
	\includegraphics[scale=0.2]{../ChapterAnalysis/figs/MELAOvertrainingCheck_example}&
	\includegraphics[scale=0.2]{../ChapterAnalysis/figs/NNOvertrainingCheck_example}&
	\includegraphics[scale=0.19]{../ChapterAnalysis/figs/ComparisonMCsEffPurity_example}\\
	\includegraphics[scale=0.2]{../ChapterAnalysis/figs/ComparisonMCsSignificance_example}&
	\includegraphics[scale=0.14]{../ChapterAnalysis/figs/FinalTrainTestROCs_example}&
	\includegraphics[scale=0.14]{../ChapterAnalysis/figs/ComparisonMCsROC_example}\\
	\end{tabular}
\end{frame}

\begin{frame}{The 3$^{rd}$ Jet Impact}
	\begin{multicols}{2}
	\begin{itemize}
		\justifying
